...
//
//      API:
//          bool ddsktx_parse(ddsktx_texture_info* tc, const void* file_data, int size,
//          ddsktx_error* err);
//              Parses texture file and fills the ddsktx_texture_info struct
//              Returns true if successfully parsed, false if failed with an error message inside
//              ddsktx_error parameter (optional) After format is parsed, you can read the contents
//              of ddsktx_format and create your GPU texture To get pointer to mips and slices see
...
//              user must provided the container object and the original file data which was passed
//              to ddsktx_parse array_idx: array index (0..num_layers) slice_face_idx: depth-slice
//              or cube-face index.
//                              if 'flags' have DDSKTX_TEXTURE_FLAG_CUBEMAP bit, then this value
//                              represents cube-face-index (0..DDSKTX_CUBE_FACE_COUNT) else it
//                              represents depth slice index (0..depth)
...
DDSKTX_API bool ddsktx_parse(ddsktx_texture_info* tc, const void* file_data, int size,
...
#define DDSKTX__DDSCAPS2_CUBEMAP_ALLSIDES                                                          \
    (0 | DDSKTX__DDSCAPS2_CUBEMAP_POSITIVEX | DDSKTX__DDSCAPS2_CUBEMAP_NEGATIVEX |                 \
     DDSKTX__DDSCAPS2_CUBEMAP_POSITIVEY | DDSKTX__DDSCAPS2_CUBEMAP_NEGATIVEY |                     \
     DDSKTX__DDSCAPS2_CUBEMAP_POSITIVEZ | DDSKTX__DDSCAPS2_CUBEMAP_NEGATIVEZ)
...
static const ddsktx__format_info k__formats_info[] = {
    {"BC1", false},     {"BC2", true},       {"BC3", true},        {"BC4", false},
    {"BC5", false},     {"BC6H", false},     {"BC7", true},        {"ETC1", false},
    {"ETC2", false},    {"ETC2A", true},     {"ETC2A1", true},     {"PTC12", false},
    {"PTC14", false},   {"PTC12A", true},    {"PTC14A", true},     {"PTC22", true},
    {"PTC24", true},    {"ATC", false},      {"ATCE", false},      {"ATCI", false},
    {"ASTC4x4", true},  {"ASTC5x5", true},   {"ASTC6x6", false},   {"ASTC8x5", true},
    {"ASTC8x6", false}, {"ASTC10x5", false}, {"<unknown>", false}, {"A8", true},
    {"R8", false},      {"RGBA8", true},     {"RGBA8S", true},     {"RG16", false},
    {"RGB8", false},    {"R16", false},      {"R32F", false},      {"R16F", false},
    {"RG16F", false},   {"RG16S", false},    {"RGBA16F", true},    {"RGBA16", true},
    {"BGRA8", true},    {"RGB10A2", true},   {"RG11B10F", false},  {"RG8", false},
    {"RG8S", false}};

static inline int ddsktx__read(ddsktx__mem_reader* reader, void* buff, int size) {
    int read_bytes =
        (reader->offset + size) <= reader->total ? size : (reader->total - reader->offset);
    ddsktx_memcpy(buff, reader->buff + reader->offset, read_bytes);
    reader->offset += read_bytes;
    return read_bytes;
}

static bool ddsktx__parse_ktx(ddsktx_texture_info* tc, const void* file_data, int size,
                              ddsktx_error* err) {
...
static bool ddsktx__parse_dds(ddsktx_texture_info* tc, const void* file_data, int size,
                              ddsktx_error* err) {
    ddsktx__mem_reader r = {(const uint8_t*)file_data, size, sizeof(uint32_t)};
    ...
    if (cubemap &&
        (header.caps2 & DDSKTX__DDSCAPS2_CUBEMAP_ALLSIDES) != DDSKTX__DDSCAPS2_CUBEMAP_ALLSIDES) {
    ...
    if (dxgi_format == 0) {
        if ((header.pixel_format.flags & DDSKTX__DDPF_FOURCC) == DDSKTX__DDPF_FOURCC) {
            int count =
                sizeof(k__translate_dds_fourcc) / sizeof(ddsktx__dds_translate_fourcc_format);
            for (int i = 0; i < count; i++) {
...
void ddsktx_get_sub(const ddsktx_texture_info* tc, ddsktx_sub_data* sub_data, const void* file_data,
                    int size, int array_idx, int slice_face_idx, int mip_idx) {
    ddsktx_assert(tc);
    ddsktx_assert(sub_data);
    ddsktx_assert(file_data);
    ddsktx_assert(size > 0);
    ddsktx_assert(array_idx < tc->num_layers);
    ddsktx_assert(!((tc->flags & DDSKTX_TEXTURE_FLAG_CUBEMAP) &&
                    (slice_face_idx >= DDSKTX_CUBE_FACE_COUNT)) &&
                  "invalid cube-face index");
    ddsktx_assert(!(!(tc->flags & DDSKTX_TEXTURE_FLAG_CUBEMAP) && (slice_face_idx >= tc->depth)) &&
    ...
    ddsktx_assert(!((tc->flags & DDSKTX_TEXTURE_FLAG_CUBEMAP) && tc->depth > 1) &&
    ...
    if (tc->flags & DDSKTX_TEXTURE_FLAG_DDS) {
        ...
        for (int mip = 0, c = tc->num_mips; mip < c; mip++) {
            ...
            ddsktx_assert(image_size == (mip_size * num_faces * num_slices) &&
                          "image size mismatch");
